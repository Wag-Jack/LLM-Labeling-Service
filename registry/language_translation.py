...
def _normalize_service_set(service_set: Iterable[dict],
    ...
    for entry in service_set:
        ...
        if name not in service_registry:
            raise KeyError(f"Unknown translation service: {name}")
        merged = {**service_registry[name], **entry}
        if merged.get("task") != "translation":
            continue
        normalized.append(merged)
    return normalized


def run_translation_services(europarl_df, service_set: Iterable[dict], use_existing: bool = False,
    ...
    for entry in services:
        ...
        if use_existing and results_file.exists():
            ...
            continue
        results[name] = runner(europarl_df, results_path=results_file)
