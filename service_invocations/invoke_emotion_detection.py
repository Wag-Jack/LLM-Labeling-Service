...
ORACLE_PROMPT = "fer_service_medium"
JUDGE_PROMPT = "fer_judge_medium"
HUMAN_LOOP_PROMPT = "fer_hitl_medium"
...
HUMAN_LOOP_NO_THRESHOLD_PROMPT = "fer_hitl_medium"
...
def _write_emotion_outputs(results_dir, label_results, oracle_results, label_df, prompt_name: str, model_name: str) -> None:
    ...
    augmented = {
        **label_results,
        LLMAAS_SERVICE: oracle_as_service(oracle_results, transform=oracle_top_emotion),
    }
    per_sample = compute_emotion_rows(augmented, oracle_results, label_df)
    service_rows, llmaas_rows = split_llmaas_rows(per_sample)
    summary = compute_emotion_summary_rows(service_rows, list(label_results.keys()))
    ...
    llmaas_summary = compute_emotion_summary_rows(llmaas_rows, [LLMAAS_SERVICE])
    ...
    if label_results and _has_oracle_results(oracle_results):
        ...
        if isinstance(oracle_results, dict):
            for model_name, model_oracle in oracle_results.items():
                _write_emotion_outputs(results_dir, label_results, model_oracle, label_df, ORACLE_PROMPT, model_name)
        else:
            _write_emotion_outputs(results_dir, label_results, oracle_results, label_df, ORACLE_PROMPT, "default")
    elif label_results:
