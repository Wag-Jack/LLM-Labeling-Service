...
def _write_comet_outputs(label_results, oracle_results, label_df, prompt_name: str, model_name: str) -> None:
    ...
    if label_results and _has_oracle_results(oracle_results):
        ...
        if isinstance(oracle_results, dict):
            for model_name, model_oracle in oracle_results.items():
                _write_comet_outputs(label_results, model_oracle, label_df, ORACLE_PROMPT, model_name)
        else:
            _write_comet_outputs(label_results, oracle_results, label_df, ORACLE_PROMPT, "default")
    elif label_results:
